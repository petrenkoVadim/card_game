...
func instantiate_hand_cards():
	...
	for i in range(0,4):
		var card_id = enemy_cards[i]
		if card_id == "": continue
		var slot = slots[i]
		var card_instance = card_scene.instantiate()
		slot.add_child(card_instance) 
		card_instance.setup(card_id, CardDB.cards[card_id], "battle", i, false, hand_enemys_skills[i])
		enemy_card_instances.append(card_instance) 
		curr_stats_enemy.append(card_instance.data.stats)
	...
	for i in range(8,16):
		var card_id = player_cards[player_index]
		if card_id == "": 
			player_index += 1
			continue
		var slot = slots[i]
		var card_instance = card_scene.instantiate()
		slot.add_child(card_instance)
		card_instance.setup(card_id, CardDB.cards[card_id], "battle", player_index, false, Global.hand_players_skills[player_index])
